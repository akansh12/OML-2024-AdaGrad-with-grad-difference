...
from optimizer import AdamWithDiff, AdaGradWithDiff, Adam, AdaGrad
import time
...
class ArtNet(object):
    ...
    def __init__(self, args, normalization_mean, normalization_std,
        ...
        self.model = vgg19(pretrained=True).features.to(device).eval() if args.model== "vgg19" else vgg19_bn(pretrained=True).features.to(device).eval()

        # self.loss_tao = args.loss_tao
    ...
    def train(self, input_img):
        ...
        self._init_optimizer(input_img, self.args.optimizer)
        ...
        while loss > self.args.loss_tao:
            def closure():
                ...
                loss = style_score.item() + content_score.item()
            ...
            if self.args.optimizer == LBFGS:
                ...
                self.optimizer.zero_grad()  #reset gradient to avoid correlation with previous batch
                model(input_img)
                style_score = 0
                content_score = 0
                ...
                loss = style_score + content_score
                loss.backward()  #compute gradient using chain rule
                _,denom = self.optimizer.step()   #update the image/parameters
                denom_dict.append(denom)
        ...
        return input_img, loss_dict, epoch, time_end - time_start, denom_dict

    def _init_optimizer(self, input_img, optimizer_type=LBFGS):
        """Initialize LBFGS optimizer."""
        if optimizer_type == ADAMDIFF:
            self.optimizer = AdamWithDiff([input_img.requires_grad_()])
        elif optimizer_type == ADAM:
            # self.optimizer = optim.Adam([input_img.requires_grad_()], lr=self.args.lr)
            self.optimizer = Adam([input_img.requires_grad_()])
        elif optimizer_type == ADADIFF:
            self.optimizer = AdaGradWithDiff([input_img.requires_grad_()])
        elif optimizer_type == ADA:
            # self.optimizer = optim.Adagrad([input_img.requires_grad_()], lr=self.args.lr)
            self.optimizer = AdaGrad([input_img.requires_grad_()])
        else:
